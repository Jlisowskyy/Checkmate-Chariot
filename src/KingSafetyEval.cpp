...
int32_t KingSafetyEval::EvalKingShelter(const Board& bd) {
    ...
    if ((bd.BitBoards[wKingIndex] & KingMap::ShelterLocationMask[WHITE]) != 0
        && CountOnesInBoard(bd.BitBoards[wPawnsIndex] & GetFrontlineMask(WHITE, ExtractMsbPos(bd.BitBoards[wKingIndex]))) < 3)
        eval += KingNoShelterPenalty;

    if ((bd.BitBoards[bKingIndex] & KingMap::ShelterLocationMask[BLACK]) != 0
        && CountOnesInBoard(bd.BitBoards[bPawnsIndex] & GetFrontlineMask(BLACK, ExtractMsbPos(bd.BitBoards[bKingIndex]))) < 3)
        eval += -KingNoShelterPenalty;
