...
void DisplayBoard(const Board& bd)
{
    ...
    static constexpr const char* castlingNames[] = {"White King Side", "White Queen Side", "Black King Side",
                                                    "Black Queen Side"};
...
std::string GetEncodingFromBoards(const Board& bd, uint64_t oldMap, uint64_t newMap, const MoveTypes mType)
{
...
std::pair<uint64_t, uint64_t> ExtractPositionsFromEncoding(const std::string& encoding)
{
    ...
    return {ExtractPosFromStr(std::toupper(encoding[0]), std::toupper(encoding[1])),
            ExtractPosFromStr(std::toupper(encoding[2]), std::toupper(encoding[3]))};
}

const std::unordered_map<size_t, Descriptors> figToDescMap{
    {'P', wPawnsIndex},   {'N', wKnightsIndex}, {'B', wBishopsIndex}, {'R', wRooksIndex},
    {'Q', wQueensIndex},  {'K', wKingIndex},    {'p', bPawnsIndex},   {'n', bKnightsIndex},
    {'b', bBishopsIndex}, {'r', bRooksIndex},   {'q', bQueensIndex},  {'k', bKingIndex}};

const std::unordered_map<size_t, char> descToFigMap{{wPawnsIndex, 'P'}, {wKnightsIndex, 'N'}, {wBishopsIndex, 'B'},
                                                    {wRooksIndex, 'R'}, {wQueensIndex, 'Q'},  {wKingIndex, 'K'},
                                                    {bPawnsIndex, 'p'}, {bKnightsIndex, 'n'}, {bBishopsIndex, 'b'},
                                                    {bRooksIndex, 'r'}, {bQueensIndex, 'q'},  {bKingIndex, 'k'}};

const std::unordered_map<Field, std::string> fieldStrMap{
    {INVALID, "XX"}, {A1, "a1"}, {B1, "b1"}, {C1, "c1"}, {D1, "d1"}, {E1, "e1"}, {F1, "f1"}, {G1, "g1"}, {H1, "h1"},
    {A2, "a2"},      {B2, "b2"}, {C2, "c2"}, {D2, "d2"}, {E2, "e2"}, {F2, "f2"}, {G2, "g2"}, {H2, "h2"}, {A3, "a3"},
    {B3, "b3"},      {C3, "c3"}, {D3, "d3"}, {E3, "e3"}, {F3, "f3"}, {G3, "g3"}, {H3, "h3"}, {A4, "a4"}, {B4, "b4"},
    {C4, "c4"},      {D4, "d4"}, {E4, "e4"}, {F4, "f4"}, {G4, "g4"}, {H4, "h4"}, {A5, "a5"}, {B5, "b5"}, {C5, "c5"},
    {D5, "d5"},      {E5, "e5"}, {F5, "f5"}, {G5, "g5"}, {H5, "h5"}, {A6, "a6"}, {B6, "b6"}, {C6, "c6"}, {D6, "d6"},
    {E6, "e6"},      {F6, "f6"}, {G6, "g6"}, {H6, "h6"}, {A7, "a7"}, {B7, "b7"}, {C7, "c7"}, {D7, "d7"}, {E7, "e7"},
    {F7, "f7"},      {G7, "g7"}, {H7, "h7"}, {A8, "a8"}, {B8, "b8"}, {C8, "c8"}, {D8, "d8"}, {E8, "e8"}, {F8, "f8"},
    {G8, "g8"},      {H8, "h8"}};
