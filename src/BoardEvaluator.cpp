...
BoardEvaluator::MaterialArrayT BoardEvaluator::_materialTable = []() -> MaterialArrayT
{
    ...
    for (size_t i = 0; i < MaterialTableSize; ++i)
    {
        ...
        for (size_t j = 0; j < kingIndex; ++j)
        {
            const int phasedFigVal =
                (BasicFigureValues[j] * (MaxTapperedCoef - actPhase) + EndGameFigureValues[j] * actPhase) /
                MaxTapperedCoef;
...
int32_t BoardEvaluator::PlainMaterialEvaluation(const Board &bd)
{
    ...
    for (size_t i = 0; i < Board::BoardsCount; ++i) eval += CountOnesInBoard(bd.boards[i]) * BasicFigureValues[i];
...
BoardEvaluator::_fieldEvalInfo_t BoardEvaluator::_evaluatePawns(
    Board &bd, const uint64_t blackPinnedFigs, const uint64_t whitePinnedFigs, const uint64_t fullMap
)
{
    _fieldEvalInfo_t rv{};
