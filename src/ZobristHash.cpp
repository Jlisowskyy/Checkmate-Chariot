...
ZobristHasher::ZobristHasher(const uint64_t engineSeed)
{
    ...
    for (size_t bd = 0; bd < Board::BoardsCount; ++bd)
        for (size_t field = 0; field < Board::BoardFields; ++field) _mainHashes[bd][field] = randEngine();

    // sentinel board should not affect
    for (size_t field = 0; field < Board::BoardFields; ++field) _mainHashes[Board::SentinelBoardIndex][field] = 0;
