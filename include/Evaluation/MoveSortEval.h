...
struct MoveSortEval
{
    ...
    static uint16_t ApplyAttackFieldEffects(uint16_t eval, const uint64_t pawnAttacks, const uint64_t startField,
    ...
    static uint16_t ApplyPromotionEffects(const uint16_t eval, const size_t nFig) { return FigureEval[nFig] + eval; }
