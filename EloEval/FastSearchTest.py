...
# install if missing
if not os.path.exists(f'{e.exes_path}/{engine_name}'):
    # change folder to installers
    print(f'{engine_name} not found, running installer...')
    os.system(f'chmod +x {e.installers_path}/{engine_name}.sh')
    os.chdir(e.installers_path)
    os.system(f'./{engine_name}.sh')
    # change folder back to root
    os.chdir('../../')

# print elo of the engine
print('running tests with the following engine:')
print(f'\t{engine_name} ({e.ext_engines[engine_name]})')
...
engine_options += f' -engine cmd={e.exes_path}/{engine_name} name={engine_name}'

print(f'./c-chess-cli {each_options} {game_options} {output_options} {engine_options}')
os.system(f'./c-chess-cli {each_options} {game_options} {output_options} {engine_options}')
